...
def prepare_update_data(batch_profile: pd.DataFrame):
    """
    Given a DataFrame of active power profile indexed by timestamps
    and columns=IDs, build the update_data dict for sym_load.
    """
    load_profile_active = initialize_array(DatasetType.update, ComponentType.sym_load, batch_profile.shape)
    load_profile_active["id"] = batch_profile.columns.to_numpy()
    load_profile_active["p_specified"] = batch_profile.to_numpy()
...
def calculate_node_stats(output_data, batch_profile: pd.DataFrame):
    ...
    min_voltage_id = np.empty(n, dtype=object)
    max_voltage_id = np.empty(n, dtype=object)
...
def main():
    ...
    update_data = prepare_update_data(active_power_profile)
